...
```{r glimpse-2}
descarga_datos_abiertos(language = "Español", parse_dictionary = FALSE) %>%
  dplyr::glimpse()
```

# Funcionamiento general

Basta componer la base de datos con cualquiera de las funciones para tener una tabla de datos agregada por fecha y entidad. Por ejemplo:
...
```{r, eval = FALSE}
datos_covid %>% 
    filter(ENTIDAD_NAC == "QUINTANA ROO" | ENTIDAD_NAC == "AGUASCALIENTES") %>%
...
datos_covid %>% 
  ...
  positividad_antigenos(fecha_tipo = "Defunción", group_by_entidad = F) %>%
  smooth() %>%
  covid_plot()
```
