...
```{r, message = FALSE, warning=FALSE}
read_datos_abiertos(tblname = "tutorial") %>% glimpse()
```
...
datos_covid <- read_datos_abiertos(diccionario_zip_path = zip_path, tblname = "tutorial")
```
...
```{r, warning = FALSE}
datos_covid$numero_pruebas %>% plot_covid(facet_ncol = 2, date_break_format = "1 month")
```
...
datos_covid %>%
  plot_covid(df_name = "CFR_HOSPITALIZADOS", type = "line", facet_ncol = 2,
...
```{r splineacional}
datos_covid %>% 
  plot_covid("Todos (nacional)", type = "spline", spar = 0.5)
```
