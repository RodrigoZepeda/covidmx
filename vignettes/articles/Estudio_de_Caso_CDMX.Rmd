...
> **Nota** No olvides que necesitas instalar [MariaDB](https://rodrigozepeda.github.io/covidmx/articles/Instalacion_de_MARIADB.html) en todas las plataformas (o tener demasiada `RAM`) y en Windows adicionalmente requieres [`7zip`](https://www.7-zip.org/) u otro descompresor de archivos `zip` que no sea el `tar` default. 

## Casos semanales y rt
...
datos_covid <- datos_covid %>%
  ...
  #Y calculamos el rt
  rt(entidades = "CIUDAD DE MÉXICO",
     min_date  = as.Date("2021/11/20", format = "%Y/%m/%d"),
     tipo_clasificacion = "Confirmados COVID",
     method = "parametric_si",        #Método de estimación del rt
...
```{r}
#Limpiamos del rt los últimas dos semanas porque se cae
datos_covid$rt <- datos_covid$rt %>%
...
cdmx_rt <- ggplot() +
  ...
  geom_line(aes(x = FECHA_SINTOMAS, 40000*`Median(R)`), data = datos_covid$rt,
...
ocupacion_estado <- descarga_datos_ocupacion_hospitalaria("Estatal")

#Descargamos las unidades médicas
ocupacion_UM     <- descarga_datos_ocupacion_hospitalaria("Unidad Médica")
```
...
plot_hospitalizados <- ggplot(datos_covid$hospitalizados) +
  geom_area(aes(x = FECHA_INGRESO, y = hospitalizados_percent/100*m_pacientes),
            fill = colores[8], alpha = 0.25) +
  geom_line(aes(x = FECHA_INGRESO, y = as.double(n)), color = colores[1]) +
  theme_minimal() +
...
ocupacion_UM <- ocupacion_UM %>%
  mutate(unidad_medica = factor(unidad_medica, 
                                levels = unidad_medica[order(hospitalizados_percent)],
                                ordered = TRUE)) %>%
  filter(hospitalizados_percent > 0)

#Reescalamos el porcentaje para que aparezca
plot_ocupacion <- ggplot(ocupacion_UM) +
  geom_col(aes(x = unidad_medica, y = hospitalizados_percent/100, fill = hospitalizados_percent)) +
  labs(
    x = "", 
...
positividad_plot <- ggplot(datos_covid$positividad) +
  geom_col(aes(x = FECHA_SINTOMAS, y = as.numeric(n_pruebas), fill = TIPO_PRUEBA), 
           alpha = 0.25) +
  geom_line(aes(x = FECHA_SINTOMAS, y = Positividad*mpruebas, color = TIPO_PRUEBA)) +
  theme_minimal() +
...
plot_defunciones <- ggplot(datos_covid$defunciones) +
  geom_col(aes(x = FECHA_DEF, y = as.numeric(n), fill = EDAD_CAT)) +
