#' DEFUNCIONES HOSPITALIZADOS
#'
#' @description
#' `defunciones_hospitalizados` Calcula el numero de casos registrados
#'  por fecha de defunción en UCI
#'
#' @details
#' Por default calcula el numero de defunciones
#' vease \code{defunciones},
#' \code{defunciones_ambulatorios} y \code{defunciones_uci}
#'
...
#'datos_covid %>% defunciones_hospitalizados()
#'
#'#Positividad en Jalisco y Colima
#'datos_covid %>% defunciones_hospitalizados(entidades = c("JALISCO","COLIMA")) %>%
#'          covid_plot()
#'
#'datos_covid %>%
#'     defunciones_hospitalizados(entidades = c("JALISCO","COLIMA"),
...
defunciones_hospitalizados <- function(datos_covid = NULL,
  ...
  defunciones_hospitalizados <- datos_covid %>%
    dplyr::filter(!is.na(get("FECHA_DEF"))) %>%
    casos_hospitalizados(entidades, group_by_entidad, entidad_tipo, fecha_tipo,
                       tipo_caso) %>%
    dplyr::rename_with(.cols = dplyr::matches("Hospitalizado"),
                       function(x) paste0("Defunciones_",x))


  return(defunciones_hospitalizados)
