#' DEFUNCIONES AMBULATORIOS
#'
#' @description
#' `defunciones_ambulatorios` Calcula el numero de casos registrados
#'  por fecha de defunción en ambulatorios
#'
#' @details
...
#'datos_covid %>% defunciones_ambulatorios()
#'
#'#Positividad en Jalisco y Colima
#'datos_covid %>% defunciones_ambulatorios(entidades = c("JALISCO","COLIMA")) %>%
#'          covid_plot()
#'
#'datos_covid %>%
#'     defunciones_ambulatorios(entidades = c("JALISCO","COLIMA"),
...
defunciones_ambulatorios <- function(datos_covid = NULL,
  ...
  defunciones_ambulatorios <- datos_covid %>%
    dplyr::filter(!is.na(get("FECHA_DEF"))) %>%
    casos_ambulatorios(entidades, group_by_entidad, entidad_tipo, fecha_tipo,
                       tipo_caso) %>%
    dplyr::rename_with(.cols = dplyr::matches("Ambulatorio"),
                       function(x) paste0("Defunciones_",x))


  return(defunciones_ambulatorios)
