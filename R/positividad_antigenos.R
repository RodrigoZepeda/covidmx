#' POSITIVIDAD POR ANTIGENOS
#'
#' @description
#' `positividad_antigenos` Calcula la positividad definida como el radio de
#' \deqn{\frac{Pruebas positivas}{Total de pruebas}} para las pruebas rapidas
#'
#' @details
#' Por default calcula la positividad total en pruebas rapidas (antigenos)
#'
#' @seealso \code{positividad_pcr} y \code{positividad}
...
#'datos_covid %>% positividad_antigenos()
#'
#'#Positividad en Jalisco y Colima
#'datos_covid %>% positividad_antigenos(entidades = c("JALISCO","COLIMA")) %>%
#'        covid_plot()
#'datos_covid %>% positividad_antigenos(entidades = c("JALISCO","COLIMA"),
#'                                 group_by_entidad = T) %>%
#'        covid_plot()
...
positividad_antigenos <- function(datos_covid = NULL,
  ...
  positividad_antigenos <- datos_covid %>%
    dplyr::filter(
    ...
    dplyr::rename_with(.cols = dplyr::matches("POSITIVIDAD"),
                       function(x) "POSITIVIDAD_ANTIGENOS")

  return(positividad_antigenos)
