#' NUMERO DE PRUEBAS PCR ESPERANDO RESULTADO
#'
#' @description
#' `numero_pruebas_esperando_resultado` Calcula el numero de pruebas que se
#' realizaron por fecha y que siguen esperando resultado
#'
#' @details
#' Por default calcula el numero de pruebas de laboratorio que siguen
#' esperando resultado por fecha
#'
...
#'datos_covid %>% numero_pruebas_esperando_resultado()
#'
#'#Positividad en Jalisco y Colima
#'datos_covid %>%
#'   numero_pruebas_esperando_resultado(entidades = c("JALISCO","COLIMA")) %>%
#'   covid_plot()
#'datos_covid %>%
#'    numero_pruebas_esperando_resultado(entidades = c("JALISCO","COLIMA"),
#'                             group_by_entidad = T) %>%
...
numero_pruebas_esperando_resultado <- function(datos_covid = NULL,
  ...
  total_pruebas <- base_por_entidad %>%
    dplyr::filter(
        stringr::str_detect(get("RESULTADO_LAB"), "PENDIENTE")
    )
  ...
  total_pruebas <- total_pruebas %>%
    ...
    dplyr::rename_with(.cols = dplyr::matches("\\bn\\b"),
                       function(x) "TOTAL_PRUEBAS_RESULTADO_PENDIENTE")
