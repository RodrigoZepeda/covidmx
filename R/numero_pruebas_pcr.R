#' NUMERO DE PRUEBAS PCR
#'
#' @description
#' `numero_pruebas_pcr` Calcula el numero de pruebas que se realizaron de
#'  tipo PCR por fecha
#'
#' @details
#' Por default calcula el numero de pruebas de laboratorio
#' vease \code{numero_pruebas_antigeno} y
#' \code{numero_pruebas} para los numeros especificos de antigeno y totales.
#'
...
#'datos_covid %>% numero_pruebas_pcr()
#'
#'#Positividad en Jalisco y Colima
#'datos_covid %>% numero_pruebas_pcr(entidades = c("JALISCO","COLIMA")) %>%
#'          covid_plot()
#'datos_covid %>% numero_pruebas_pcr(entidades = c("JALISCO","COLIMA"),
#'                             group_by_entidad = T) %>%
#'          covid_plot()
...
numero_pruebas_pcr <- function(datos_covid = NULL,
  ...
  total_pruebas_pcr <- datos_covid %>%
    dplyr::filter(
      stringr::str_detect(get("RESULTADO_LAB"),
                          paste0("NO POSITIVO A SARS-COV-2|POSITIVO A ",
                                 "SARS-COV-2|RESULTADO NO ADECUADO|RESULTADO ",
                                 "PENDIENTE"))) %>%
    numero_pruebas(entidades, group_by_entidad, entidad_tipo, fecha_tipo) %>%
    dplyr::rename_with(.cols = dplyr::matches("TOTAL_PRUEBAS"),
                       function(x) "TOTAL_PRUEBAS_PCR")

  return(total_pruebas_pcr)
